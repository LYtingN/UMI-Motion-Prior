...
# This container ships a 64MB /dev/shm (Docker default). PyTorch's default
# multiprocessing sharing strategy ('file_descriptor') passes DataLoader tensors
# through that shm segment, so num_workers>0 dies with "Bus error / out of shared
# memory" once a few workers prefetch. 'file_system' backs shared tensors with
# /tmp files instead, lifting the 64MB cap and letting num_workers>0 work.
# Harmless when num_workers=0 (no worker processes are spawned).
torch.multiprocessing.set_sharing_strategy("file_system")


def _find_repo_root(path: Path) -> Path:
    """Walk up from ``path`` until a directory containing ``Prior_Recon/`` is found."""
...
from Prior_Recon.Masked_Flow.dataset.config import g1sonic_delta_masked_flow_config
from Prior_Recon.Masked_Flow.dataset.delta_dataset import (
    G1DeltaFeatPrimitiveDataset,
)
from Prior_Recon.Masked_Flow.trainer_masked_flow import MaskedFlowTransformerTrainer
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train Masked-Flow delta69 model",
    ...
    parser.add_argument(
        "--segment-unrolls", type=int, default=None, metavar="N",
        help="Override cfg.primitive.segment_unrolls, including when resuming a checkpoint.",
    )
    parser.add_argument(
...
def main() -> None:
    ...
    if args.resume:
        ...
        cfg = resume_state.get("cfg", g1sonic_delta_masked_flow_config())
        if args.config:
        ...
        if args.config:
            ...
            cfg = g1sonic_delta_masked_flow_config()
    ...
    if args.segment_unrolls is not None:
        if args.segment_unrolls < 1:
            raise ValueError("--segment-unrolls must be at least 1")
        cfg.primitive.segment_unrolls = args.segment_unrolls
    ...
    trainer = MaskedFlowTransformerTrainer(
        cfg,
        device=args.device,
